...
#include "brand_png_256.h"  // generated by tb_embed from assets/brand/tacklebox-256.png
#endif
...
void handleDrop(const char* dropped) {
    ...
    // macOS hands a tacklebox:/esr: url opened while we run to SDL as a
    // "dropped file" (LaunchServices -> Apple event -> SDL_EVENT_DROP_FILE).
    if (tb::deeplink::parse(path).kind != tb::deeplink::Kind::None) {
        g_controller->handleDeepLink(path);
        return;
    }
...
#if defined(__linux__)
    // A portable/AppImage copy writes its own launcher entry; the mark goes
    // with it so the menu shows an icon.
    tb::deeplink::registerSchemes(brand_png_256, brand_png_256_size);
#else
